...
// Function to convert ELF flags to mmap protection flags
...
// Function to get mapping flags for mmap
int get_mapping_flags(Elf32_Word p_type) {
    // For loading segments, we typically use MAP_FIXED and MAP_PRIVATE
    if (p_type == PT_LOAD) {
...
// Function to format protection flags as string
void get_prot_string(int prot_flags, char *prot_str) {
    prot_str[0] = (prot_flags & PROT_READ) ? 'R' : ' ';
    prot_str[1] = (prot_flags & PROT_WRITE) ? 'W' : ' ';
    prot_str[2] = (prot_flags & PROT_EXEC) ? 'X' : ' ';
    prot_str[3] = '\0';
}

// Function to format mapping flags as string
void get_map_string(int map_flags, char *map_str) {
    if (map_flags & MAP_FIXED && map_flags & MAP_PRIVATE) {
        sprintf(map_str, "FIXED|PRIVATE");
    } else if (map_flags & MAP_PRIVATE) {
        sprintf(map_str, "PRIVATE");
    } else {
        sprintf(map_str, "OTHER");
    }
}

// Function to print program header information in readelf format with mmap info
void print_phdr_details(Elf32_Phdr *phdr, int index) {
    ...
    int map_flags = get_mapping_flags(phdr->p_type);
    
    get_prot_string(prot_flags, prot_str);
    get_map_string(map_flags, map_str);
    
    printf("%-8s 0x%06x 0x%08x 0x%08x 0x%05x 0x%05x %-3s 0x%-4x",
    ...
    // Only show mmap info for loadable segments
    if (phdr->p_type == PT_LOAD) {
        printf(" | Protection: %s (0x%x) | Mapping: %s (0x%x)",
               prot_str, prot_flags, map_str, map_flags);
    }
...
int main(int argc, char *argv[]) {
    ...
    printf("  Type           Offset   VirtAddr   PhysAddr   FileSiz MemSiz  Flg Align    | mmap Info\n");
