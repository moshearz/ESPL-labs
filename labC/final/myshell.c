...
void executePipe(cmdLine* pCmdLine){
    ...
    // Add Child 1 to the process list
    //addProcess(&process_list, pCmdLine, pid_child1);
    //Back to parent
    ...
    // Add Child 2 to the process list
    addProcess(&process_list, pCmdLine, pid_child1);
    cmdLine* temp = pCmdLine->next;
    pCmdLine->next = NULL; // disconnect the second command from the first one
    addProcess(&process_list, temp, pid_child2); // add the second command to the process list
